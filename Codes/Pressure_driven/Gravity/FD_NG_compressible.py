...
T = 40*24*3600           # final time
num_steps = 240*save_itr    # number of time steps
...
# Non-homogen K
# k_subdomain = conditional(Or(And(And(x>(9*hx),x<(12*hx)),And(y>3*hy,y<6*hy)),\
#                              And(And(x>(9*hx),x<(12*hx)),And(y>9*hy,y<12*hy))),1e-15,1e-12)
k_subdomain = conditional(Or(Or(Or(And(And(x>9*hx,x<12*hx),And(y>3*hy,y<6*hy)),\
                             And(And(x>9*hx,x<12*hx),And(y>9*hy,y<12*hy))),\
                             Or(And(And(x>21*hx,x<24*hx),And(y>3*hy,y<6*hy)),\
                             And(And(x>21*hx,x<24*hx),And(y>9*hy,y<12*hy)))),\
                             Or(And(And(x>33*hx,x<36*hx),And(y>3*hy,y<6*hy)),\
                             And(And(x>33*hx,x<36*hx),And(y>9*hy,y<12*hy)))
                             ),1e-15,1e-12)
#
...
Cw = Constant(1e-10)
# Cl = Constant(1e-6) # in all other tests
Cl = Constant(2.5e-7)

# Cr = Constant(0) # This value should be taken (3~6e-6 PSI^-1) 4.35e-10 to 8.7e-10
# Cw = Constant(0)
# Cl = Constant(0)
...
file_p = File("./Output/Compressible/NG_pres+FL+SL.pvd")
file_s = File("./Output/Compressible/NG_sat+FL+SL.pvd")
file_v = File("./Output/Compressible/NG_vel+FL+SL.pvd")
...
for nIter in range(num_steps):
    ...
    rhow0_v = 1000
    g_v = Constant((0,-9.8))
    v0 = Function(vSpace).interpolate(-1*lmbda_w(s0)*(grad(p0)-rhow0_v*(1+Cw*(p0))*g_v))
