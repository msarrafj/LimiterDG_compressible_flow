...
class flux_limiter(SignFlip):
    def __init__(self,mesh,sol_0,sol,rPAvg_0,rPAvg,rhoAvg_0,q_I,q_P,fluxes,solMax,solMin,dt):
        self.mesh = mesh
        self.sol_0 = sol_0
        self.sol = sol
        self.rhoAvg_0 = rhoAvg_0.vector().array()
    ...
    def apply(self):
        ...
        for numIter in range(maxIter):
            ...
            solAvg =   solAvg - self.dt/area * \
                    (1./self.rPAvg_0)* np.multiply(alphaEface,FLUX).sum(axis=1) +\
                    Corrector * self.dt * (1./self.rPAvg_0) * self.wells_update(solAvg)
